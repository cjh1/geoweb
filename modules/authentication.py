...
class ESGFSessionAuth(cptools.SessionAuth):
    ...
    def check_username_and_password(self, username, password):
        cherrypy.log('checking')
        try:
            esgf.authenticate(username, password)
        except MyProxyClientError as myex:
            cherrypy.log(str(myex))
            cherrypy.log(str(type(myex)))
    ...
    def login_screen(self, from_page='..', username='', error_msg=''):
        ...
        cherrypy.log("username: " + username)
        cherrypy.log("error: " + error_msg)
        cherrypy.log("from_page: " + from_page)
    ...
    def do_check(self):
          ...
          cherrypy.log('path: ' + request.path_info)
